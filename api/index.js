...
async function adminRoute(req,res,path,body) {
  ...
  if (path === '/dashboard' && req.method === 'GET') { const result=await query("SELECT COUNT(*)::int total, COUNT(*) FILTER(WHERE status='pending')::int pending, COUNT(*) FILTER(WHERE status='approved')::int approved, COUNT(*) FILTER(WHERE status='rejected')::int rejected, COUNT(*) FILTER(WHERE whitelist_slot IS NOT NULL)::int assigned, COUNT(*) FILTER(WHERE created_at::date=CURRENT_DATE)::int today, COUNT(DISTINCT wallet_normalized)::int unique_wallets FROM applications"); const capacity=Number(await setting('general','wl_spots','0')); return json(res,200,{success:true,stats:{...result.rows[0],capacity,available:capacity>0?Math.max(0,capacity-result.rows[0].assigned):null}}); }
