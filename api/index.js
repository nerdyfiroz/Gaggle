...
async function publicRoute(req, res, path, body) {
  ...
  if (path === '/applications' && req.method === 'POST') {
    ...
    const twitter = clean(body.twitter_username).replace(/^@/, '').toLowerCase(); const email = clean(body.email).toLowerCase();
