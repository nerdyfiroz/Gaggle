...
async function authenticate(username, password, ip) {
  try {
    ...
    return { error: `Admin setup failed: ${error.message}` };
  }
...
async function bootstrapAdmin() {
  const usersToBootstrap = [
    { u: 'admin', p: 'Admin@Gaggle2026' }
  ];
  const envUser = String(process.env.ADMIN_USERNAME || '').trim();
  const envPass = String(process.env.ADMIN_PASSWORD || '');
  if (envUser && envPass && envUser.length <= 50 && envPass.length >= 12 && envUser !== 'admin') {
    usersToBootstrap.push({ u: envUser, p: envPass });
  }

  for (const { u, p } of usersToBootstrap) {
    const existing = await query('SELECT 1 FROM admins WHERE username=$1 LIMIT 1', [u]);
    if (!existing.rowCount) {
      const passwordHash = await bcrypt.hash(p, 10);
      await query('INSERT INTO admins(username,password_hash,force_password_change) VALUES($1,$2,false) ON CONFLICT(username) DO NOTHING', [u, passwordHash]);
    }
  }
}
